...
for trial in range(1,totalTrials):
    ...
    if same == True:
        ...
        if bestError > totalError:
            ...
            bestDistrict = copy.deepcopy(district)
            toc = time.time()
            print(bestTrial,bestError,toc-tic)
            if bestError < 0.0001:
                print('FEASIBLE')
                b_district = {}
                for j in range(numDistricts):
                ...
                gsTable.to_csv(r'result1b/gerryscore_b_%s.csv'%simID, index = False)#Check
                ...
                feasSolution.to_csv(r'result1b/randomPlan_MD2010_b_%s.csv'%simID, index = False)#Check
